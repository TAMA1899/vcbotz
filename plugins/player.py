...
async def add_to_playlist(_, message: Message):
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if type in ["video", "audio"]:
            ...
            if type=="youtube":
                await msg.edit("🎧 **Fetching Video From Servers...**")
                url=yturl
            elif type=="query":
                try:
                    await msg.edit("🎧 **Fetching Video From Servers...**")
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️ASK FOR HELP', url='https://t.me/i_am_albin_praveen')]]))
    await delete_messages([m, k])
